#ifndef _EDITOR_HELPERS_H_
#define _EDITOR_HELPERS_H_
...
public:
    ...
    public:
        // Constructor
        inline iterator( tanita2_dict & dict, int i ) : dict(dict), index(i) {}
    ...
    //! Dictionary length
    inline int len() { return _keys.size(); }
...
protected:
    ...
    // Change key
    void change_key( bp::str & old_key, bp::str & new_key );
    
    // Get value by index
    bp::object & get_by_index( unsigned int index );
    // Get key by index
    bp::str & get_key_by_index( unsigned int index );
    // Get index of given key
    unsigned int get_index( bp::str & key );
