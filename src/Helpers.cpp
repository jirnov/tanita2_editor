...
bp::object tanita2_dict::iteritems()
{
    bp::list t;
    for (unsigned int i = 0; i < _keys.size(); ++i)
        t.append(make_tuple(_keys[i], _values[i]));
    return t;
}
...
// Change key
void tanita2_dict::change_key( bp::str & old_key, bp::str & new_key )
{
    for (unsigned int i = 0; i < _keys.size(); ++i)
        if (old_key == _keys[i])
        {
		...
		throw Exception(boost::str(boost::format("Invalid key: %s") % bp::extract<char *>(old_key)));
}

// Get value by index
bp::object & tanita2_dict::get_by_index( unsigned int index )
{
    if (0 > index || _keys.size() <= index)
        throw Exception("Illegal index");
    return _values[index];
}

// Get key by index
bp::str & tanita2_dict::get_key_by_index( unsigned int index )
{
    if (0 > index || _keys.size() <= index)
        throw Exception("Illegal index");
    return _keys[index];
...
unsigned int tanita2_dict::get_index( bp::str & key )
{
    for (unsigned int i = 0; i < _keys.size(); ++i)
        if (key == _keys[i])
            return i;
	throw Exception(boost::str(boost::format("Invalid key: %s") % bp::extract<char *>(key)));
}
...
// Create bindings
void tanita2_dict::create_bindings()
{
    using namespace bp;
    ...
    class_<tanita2_dict>("tanita2_dict")
        ...
        .def("change_key",   &tanita2_dict::change_key)
        .def("get_index",    &tanita2_dict::get_index)
        .def("get_by_index", &tanita2_dict::get_by_index, return_value_policy<copy_non_const_reference>())
        .def("get_key_by_index", &tanita2_dict::get_key_by_index, 
